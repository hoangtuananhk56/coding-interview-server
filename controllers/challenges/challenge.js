...
const deleteChallenge = async (req, res) => {
  await Challenge.findByIdAndDelete({ _id: req.params.id }) // conditition
    .then((challenges) => {
      if (!challenges) {
        return res
          .status(404)
...
const getChallengeById = async (req, res) => {
  await Challenge.findById({ _id: req.params.id }) // conditition
    .then((challenges) => {
      if (!challenges) {
        return res
          .status(404)
...
const getChallenges = async (req, res) => {
  ...
  await Challenge.find() // conditition
...
const searchChallenges = async (req, res) => {
  ...
  await Challenge.find({ name: { $regex: req.params.name } }) // conditition
    .skip(perPage * page - perPage)
    .limit(perPage)
    .sort({ updatedAt: -1 })
    .then((challenges) => {
