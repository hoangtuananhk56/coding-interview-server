...
const deleteComment = async (req, res) => {
  await Comment.findByIdAndDelete({ _id: req.params.id }) // conditition
    .then((comments) => {
      if (!comments) {
        return res
          .status(404)
...
const getCommentById = async (req, res) => {
  await Comment.findById({ _id: req.params.id }) // conditition
    .then((comments) => {
      if (!comments) {
        return res
          .status(404)
...
const getComments = async (req, res) => {
  await Comment.find({ candidate_id: req.params.id }) // conditition
    .then((comments) => {
      if (!comments.length) {
        return res
          .status(404)
