...
const createCandidate = async (req, res) => {
  ...
  if (!email)
    return res.status(400).json({
      success: false,
      message: "Missing candidatename and/or password",
    });
...
const deleteCandidate = async (req, res) => {
  await Candidate.findOneAndDelete({ _id: req.params.id }) // conditition
...
const getCandidateById = async (req, res) => {
  await Candidate.findById({ _id: req.params.id }) // conditition
    .then((candidates) => {
      if (!candidates) {
        return res
          .status(404)
...
const getCandidates = async (req, res) => {
  ...
  await Candidate.find() // conditition
    .skip(perPage * page - perPage)
    .limit(perPage)
    .sort({ updatedAt: -1 })
    .then((candidates) => {
...
const searchCandidates = async (req, res) => {
  ...
  await Candidate.find({ email: { $regex: req.params.email } }) // conditition
    .skip(perPage * page - perPage)
    .limit(perPage)
    .sort({ updatedAt: -1 })
    .then((candidates) => {
